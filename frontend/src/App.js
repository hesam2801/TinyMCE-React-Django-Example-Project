...
function App() {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/app/view/").then(res => setData(res.data.data))
  }, 0)
  ...
  return (
    <>
      <div>
        ...
        <Editor
          apiKey='hawyq4vj957kiud7vcb6p48lvpr0zwl5i57j9ar7lj5atuft'
          init={{
            // plugins: [
        ...
        <div className="buttons">
          <input value="save" type="submit" onClick={() => {
            log()
            if (editing == 0) {
              let output = {
              ...
              axios.post("http://127.0.0.1:8000/app/edit/" + editing + "/", output)
              setEditing(0)
              txtName.current.value = ''
              editorRef.current.setContent('')
            }

            axios.get("http://127.0.0.1:8000/app/view/").then(res =>{alert(res.data.message); setData(res.data.data)})
          }} />
          <input type="button" value="cancel" style={{ display: editing != 0 ? "block" : "none" }} onClick={() => {
            setEditing(0)
            txtName.current.value = ''
            editorRef.current.setContent('')
          }} />
